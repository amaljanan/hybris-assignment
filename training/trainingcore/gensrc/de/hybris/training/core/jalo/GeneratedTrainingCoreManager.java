 ...
 * --- Generated at 11-Jul-2021, 8:12:54 PM                     ---
 * ----------------------------------------------------------------
 *  
...
import de.hybris.training.core.jalo.TrainingCatalog;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public TrainingCatalog createTrainingCatalog(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGCATALOG );
			return (TrainingCatalog)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating TrainingCatalog : "+e.getMessage(), 0 );
		}
	}
	
	public TrainingCatalog createTrainingCatalog(final Map attributeValues)
	{
		return createTrainingCatalog( getSession().getSessionContext(), attributeValues );
	}
	
	public TrainingSample createTrainingSample(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGSAMPLE );
			return (TrainingSample)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating TrainingSample : "+e.getMessage(), 0 );
		}
	}
	
	public TrainingSample createTrainingSample(final Map attributeValues)
	{
		return createTrainingSample( getSession().getSessionContext(), attributeValues );
	}
	...
	public TrainingUser createTrainingUser(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.TRAININGUSER );
